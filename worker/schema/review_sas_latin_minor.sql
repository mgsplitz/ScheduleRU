-- Reviewed SAS New Brunswick Latin minor (program code 580).
...
VALUES ('sasnb-latin-minor','Latin','sasnb','latin-minor','minor','Current official departmental and SAS pages','580',NULL,'sasnb-latin-580','https://classics.rutgers.edu/academics/undergraduate/majors-minors','reviewed',strftime('%s','now')*1000,1)
...
('sasnb-latin-minor','https://classics.rutgers.edu/academics/undergraduate/majors-minors','Classics Majors & Minors',NULL,'program_requirements',strftime('%s','now')*1000,'Current Classics department minor requirements page.'),
('sasnb-latin-minor','https://sasundergrad.rutgers.edu/majors-and-core-curriculum/major/list-of-majors-and-minors','SAS Majors and Minors',NULL,'program_profile',strftime('%s','now')*1000,'Current SAS list confirms Latin is an SAS minor.')
...
DELETE FROM program_requirement_evidence WHERE program_id='sasnb-latin-minor';
DELETE FROM requirement_course_selectors WHERE group_id IN (SELECT id FROM requirement_groups WHERE program_id='sasnb-latin-minor');
DELETE FROM requirement_courses WHERE group_id IN (SELECT id FROM requirement_groups WHERE program_id='sasnb-latin-minor');
DELETE FROM requirement_groups WHERE program_id='sasnb-latin-minor';
DELETE FROM program_eligibility_rules WHERE program_id='sasnb-latin-minor';

INSERT INTO requirement_groups (id,program_id,parent_group_id,name,rule,count,sort_order,auto_generated) VALUES
('sasnb-latin-minor-total','sasnb-latin-minor',NULL,'Six Latin language courses.','min_courses',6,10,0),
('sasnb-latin-minor-upper-level','sasnb-latin-minor','sasnb-latin-minor-total','At least three Latin language courses at the 300 level or higher.','min_courses',3,10,0);
...
('sasnb-latin-minor-total','sasnb-latin-minor-courses','{"version":1,"kind":"subject_level","school_codes":["01"],"subject_codes":["580"],"course_number_min":100,"course_number_max":499,"label":"Rutgers-New Brunswick Latin course"}','https://classics.rutgers.edu/academics/undergraduate/majors-minors','Classics Majors & Minors','reviewed',strftime('%s','now')*1000),
('sasnb-latin-minor-upper-level','sasnb-latin-minor-upper-courses','{"version":1,"kind":"subject_level","school_codes":["01"],"subject_codes":["580"],"course_number_min":300,"course_number_max":499,"label":"300- or 400-level Rutgers-New Brunswick Latin course"}','https://classics.rutgers.edu/academics/undergraduate/majors-minors','Classics Majors & Minors','reviewed',strftime('%s','now')*1000)
...
SELECT 'group:'||id,'sasnb-latin-minor','group',id,NULL,'https://classics.rutgers.edu/academics/undergraduate/majors-minors','Classics Majors & Minors',NULL,strftime('%s','now')*1000,'Current Latin minor requirement.','reviewed'
FROM requirement_groups WHERE program_id='sasnb-latin-minor';
...
('sasnb-latin-minor-academic-review','sasnb-latin-minor','advisor_confirmation','{"topics":["a grade of C or better is required for every course counted toward the minor"]}','requires_approval','Confirm the published course-grade rule with Classics advising before relying on this plan.','https://classics.rutgers.edu/academics/undergraduate/majors-minors','reviewed',strftime('%s','now')*1000)
